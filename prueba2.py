...
# Mostrar equipos con escudos y definir fair play inicial
fair_play_fijo = {
    "Grupo C": {
        (0, 3): {0: (0, 0, 0), 3: (0, 0, 0)},
        (0, 2): {0: (1, 0, 0), 2: (3, 0, 0)},
...
# Inicializar fair play con datos fijos
disciplinas = {team: 0 for team in team_names}

if grupo in fair_play_fijo:
    for (a, b), tarjetas in fair_play_fijo[grupo].items():
        for idx, (amarillas, dobles, rojas) in tarjetas.items():
            team = team_names[idx]
            puntos = -1 * amarillas - 3 * dobles - 4 * rojas
            disciplinas[team] += puntos
...
def desempatar_fifa(equipos, match_results, global_stats, historial=None):
    ...
    if empate_total:
        ...
        return criterios_d_e_f(global_stats, equipos, disciplinas)
...
def criterios_d_e_f(df, equipos_empatados, disciplinas):
    sub_df = df[df["Equipo"].isin(equipos_empatados)].copy()
    sub_df["FairPlay"] = sub_df["Equipo"].map(disciplinas)
    sub_df = sub_df.sort_values(by=["DG", "GF", "FairPlay"], ascending=[False, False, False])
...
if all(team_names):
    ...
    for i, (a, b) in enumerate(matchups):
        ...
        if grupo in resultados_fijos and (a, b) in resultados_fijos[grupo]:
            ...
            match_results.append((team_names[a], g1, g2, team_names[b]))
             # Inputs de tarjetas para team_names[a]
            st.markdown(f"🟨 Tarjetas {team_names[a]}")
            a_yellow = st.number_input(f"{team_names[a]} - Amarillas", min_value=0, key=f"ay_{i}")
            a_double = st.number_input(f"{team_names[a]} - Doble amarilla", min_value=0, key=f"ad_{i}")
            a_red = st.number_input(f"{team_names[a]} - Roja directa", min_value=0, key=f"ar_{i}")

                # Inputs de tarjetas para team_names[b]
            st.markdown(f"🟥 Tarjetas {team_names[b]}")
            b_yellow = st.number_input(f"{team_names[b]} - Amarillas", min_value=0, key=f"by_{i}")
            b_double = st.number_input(f"{team_names[b]} - Doble amarilla", min_value=0, key=f"bd_{i}")
            b_red = st.number_input(f"{team_names[b]} - Roja directa", min_value=0, key=f"br_{i}")

                # Calcular puntaje fair play por equipo
            fair_a = -1 * a_yellow -3 * a_double -4 * a_red
            fair_b = -1 * b_yellow -3 * b_double -4 * b_red

            disciplinas[team_names[a]] = disciplinas.get(team_names[a], 0) + fair_a
            disciplinas[team_names[b]] = disciplinas.get(team_names[b], 0) + fair_b
    ...
    for puntos, grupo_puntos in df_sorted.groupby("Puntos", sort=False):
        if len(grupo_puntos) == 1:
            ...
            if len(set(equipos_ordenados)) < len(equipos_empatados):
                equipos_ordenados = criterios_d_e_f(df, equipos_empatados, disciplinas)
    ...
    orden_final_df["Fair Play"] = orden_final_df["Equipo"].apply(
    lambda t: disciplinas.get(t.split('"> ')[-1], 0)
)
